...
for i,v in ipairs(maps) do
	local map = v.data
	if not v.style then
		local f = io.open("/Users/ckknight/wmo/" .. map .. ".wmo", "rb")
		if not f then
			error("Cannot find map file: /Users/ckknight/wmo/" .. map .. ".wmo")
		end
		v.text = f:read("*all")
		f:close()
	end
...
print("Cartographer3.SetInstanceTextureData {")
for i,v in ipairs(maps) do
	local name = v.name
	local map = v.data
	local lines = {}
